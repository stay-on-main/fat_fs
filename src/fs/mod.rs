...
impl <T: StorageIo> Fs <T> {
    ...
    pub fn dir_entry_find(&self, path: &str) -> Result<DirEntry, bool> {
        let mut dir_entry = DirEntry::root(self.root_dir_cluster());
        let mut found = false; 

        for x in Path::new(path) {
            //print_str(x);
            if !dir_entry.is_dir() {
                return Err(false);
            }
    ...
    pub fn file_open(&self, path: &str) -> Result<File<T>, bool> {
        let file = self.dir_entry_find(path)?;
        
        if file.is_file() {
            Ok(File::new(Stream::new(self, file.cluster, false), file.size()))
        } else {
            Err(false)
        }
    }
